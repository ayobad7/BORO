          ...
          {activities.length > 0 ? (
            (() => {
              ...
              list.push(
                ...
                if (showPeople && favOwners.length > 0) {
                  const people = (
                    <PeopleCard key='people-card' owners={favOwners} />
                  );
                  // If latest favorite is most recent overall, place first; else place as 3rd card
                  const latestActivityTs = Math.max(
                    ...activities.map((a) => a.activityTimestamp)
                  );
                  if (latestFavAt && latestFavAt > latestActivityTs) {
                    // If newest, place PeopleCard right after RecentActivityCard
